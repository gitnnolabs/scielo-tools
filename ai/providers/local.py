...
from ai.providers import JSON_INSTRUCTION
...
class LocalProvider:
    ...
    def __init__(self, model, response_format=None, temperature=0.0, top_p=0.1, max_tokens=4000, stop=None, n_ctx=None, nthreads=2):
        self.model = model
        ...
        if LocalProvider._cached_llm is None:
            try:
                ...
                raise LlamaNotInstalledError("llama-cpp-python not installed.") from e

            model_path = os.path.join(LLAMA_MODEL_DIR, self.model.name_file)
    ...
    def prompt(self, user_input, response_format=None):
        ...
        try:
            ...
            response_text = ""
        logger.info("Local Llama prompt: %d chars in %.2fs", len(response_text), elapsed)
