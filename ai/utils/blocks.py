from ai.utils.normalizers import stz_language
...
def block_text(value):
    text = value.get("paragraph") or value.get("text_aff") or value.get("original") or value.get("title") or ""
...
def make_lang_block(label, text, language):
    return {
        ...
        "value": {"label": label, "language": stz_language(language, "en"), "paragraph": text or ""},
    }


def source_blocks(front, body, limit=60):
    blocks = []
    for section, items in (("front", front), ("body", body[:25] if isinstance(body, list) else body)):
        for index, item in enumerate(items or []):
            ...
            blocks.append({
                ...
                "type": block_type, "label": value.get("label", ""),
                "text": text[:max_text],
            })
            if len(blocks) >= limit:
                return blocks
