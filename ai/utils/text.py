...
def extract_text_from_docx(docx_path, limit_chars=30000):
    ...
    paragraphs = []
    for p in root.xpath("//w:p | //w:tc//w:p", namespaces=nsmap):
        text = "".join(t.text or "" for t in p.xpath(".//w:t", namespaces=nsmap))
        text = text.strip()
        if text:
            paragraphs.append(text)

    result = "\n".join(paragraphs)[:limit_chars]
    logger.info("Text extractor: zipfile produced %d chars, preview: %s", len(result), result[:200])
    return result
...
def extract_text_from_docx_via_docling(docx_path, limit_chars=30000):
    ...
    logger.info("Text extractor: docling produced %d chars, preview: %s", len(result_text), result_text[:200])
    return result_text
