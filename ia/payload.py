...
from ia.utils.normalizers import (
    DOI_RE,
    ORCID_RE,
    stz_affiliation_id,
    stz_country_code,
...
def normalize_payload(payload, article):
    ...
    for item in payload.get("titles") or []:
        ...
        if key in seen_titles:
            continue
        seen_titles.add(key)
        normalized["titles"].append({"text": text, "language": language, "kind": kind})
        ...
        normalized["authors"].append(
            {
                ...
                "affiliations": stz_affiliation_id(item.get("affiliations"))
                or [str(index)],
    ...
    for item in payload.get("keywords") or []:
        if isinstance(item, str):
            continue
        terms = [stz_text(term) for term in item.get("terms") or []]
    ...
    if normalized["titles"] and not any(
        title["kind"] == "main" for title in normalized["titles"]
    ):
        normalized["titles"][0]["kind"] = "main"
    normalized["titles"].sort(key=lambda title: 0 if title["kind"] == "main" else 1)
