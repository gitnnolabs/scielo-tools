...
class IAMetadataModel(models.Model):
...
class HuggingFaceModel(IAMetadataModel):
    ...
    panels = [
        ...
        FieldPanel(
            "download_status",
            widget=forms.Select(
                choices=DownloadStatus.choices, attrs={"disabled": True}
            ),
        ),
        FieldPanel("is_active"),
    ]
    base_form_class = IAAdminModelForm
...
class OllamaModel(IAMetadataModel):
    url = models.URLField(_("Ollama URL"), help_text="e.g. http://host:11434")
    model = models.CharField(
        _("Model"), max_length=255, blank=True, help_text="Select after fetching"
    )
    ...
    panels = [
        FieldPanel("url"),
        FieldPanel(
            "model", widget=forms.Select(attrs={"data-ai": "ollama-model-select"})
        ),
...
class GeminiModel(IAMetadataModel):
