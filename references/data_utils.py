...
from ai.references import mark_references
from references.models import ElementCitation, Reference, ReferenceStatus
...
def get_reference(obj_id):
    logger.info("Iniciando tarefa get_reference para ID=%s", obj_id)
    try:
        obj_reference = Reference.objects.get(id=obj_id)
        logger.info("Executando marcação da citação: %r", obj_reference.mixed_citation)
        ...
        for item in marked:
            for i in item['choices']:
                ...
                citations_created += 1
                logger.debug("Criado ElementCitation ID=%s para a citação marcada: %s", citation.pk, i)

        obj_reference.status = ReferenceStatus.READY
        obj_reference.save()
        logger.info("Tarefa get_reference concluída com sucesso para ID=%s. Total de citações criadas: %d", obj_id, citations_created)
    except Exception as e:
        logger.error("Erro na tarefa get_reference para ID=%s: %s", obj_id, e, exc_info=True)
        raise
